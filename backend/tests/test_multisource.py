"""Multi-source behavior: structured sources (AS24) populate fields + skip Haiku."""
...
async def test_as24_crawl_populates_fields_and_marks_normalized(test_engine):
    ...
    with Session(get_engine()) as s:
        ...
        # Structured source → make/model filled and normalized_at set (Haiku skipped).
        assert all(r.make and r.model for r in rows)
        assert all(r.normalized_at is not None for r in rows)
